...
from dy_pb2 import LiveShoppingMessage
...
def onMessage(ws: websocket.WebSocketApp, message: bytes):
    ...
    for msg in payloadPackage.messagesList:
        ...
        if msg.method == 'WebcastProductChangeMessage':
            ...
            continue
        if msg.method == 'WebcastLiveShoppingMessage':
            WebcastLiveShoppingMessage(msg.payload)
            continue
...
def WebcastProductChangeMessage(data):
    ...
    with open("ProductChange.json","a+") as f:
        f.write(str(log))

def WebcastLiveShoppingMessage(data):
    CommonTextMessage = LiveShoppingMessage()
    CommonTextMessage.ParseFromString(data)
    data = json_format.MessageToDict(CommonTextMessage, preserving_proto_field_name=True)
    log = json.dumps(data, ensure_ascii=False)
    logging.info('[WebcastLiveShoppingMessage] [] [房间Id：' + liveRoomId + '] ｜ ' + log)
    with open("liveShopping.json","a+") as f:
        f.write(str(log))
...
def unPackWebcastSocialMessage(data):
    ...
    with open("SocialMessage.json","a+") as f:
        f.write(str(log))
    return data
...
def unPackWebcastChatMessage(data):
    ...
    with open("ChatMessage.json","a+") as f:
        f.write(str(data))
    return data
...
def unPackWebcastGiftMessage(data):
    ...
    with open("GiftMessage.json","a+") as f:
        f.write(str(log))
    return data
...
def unPackWebcastMemberMessage(data):
    ...
    with open("MemberMessage.json","a+") as f:
        f.write(str(log))
    return data
...
def unPackWebcastLikeMessage(data):
    ...
    with open("LikeMessage.json","a+") as f:
        f.write(str(log))
    return data
