...
def intergrate_video(x, video_path, fragment_num, left_fragments, right_fragments, fragment_time, integrated_video_dir):
    ...
    for i in range(left_fragments):
        if i > x:
            input_files.append(video_path + (4-len(str(x)))*"0" + str(x + fragment_num - i) + ".flv")
        else:
...
if __name__ == "__main__":
    ...
    left_fragments = 16 #当检测到敏感词时，保存的过往片段个数, 请注意，该数不能大于缓冲区片段个数！
    right_fragments = 16 #当检测到敏感词时，保存的未来片段个数，请注意，该数不能大于缓冲区片段个数！
    ...
    while(True):
        ...
        if len(sensitive_video_queue) != 0:
            x = sensitive_video_queue.pop(0)
            Process(target=intergrate_video, args=(x, video_path, fragment_num, left_fragments, right_fragments, fragment_time, integrated_video_dir))
